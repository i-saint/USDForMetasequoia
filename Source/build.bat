...
:DoBuild
    ...
    msbuild mqusdPlayer.vcxproj /t:Build /p:Configuration=Release /p:Platform=Win32 /m /nologo
    IF %ERRORLEVEL% NEQ 0 (
        pause
        exit /B 1
    )
    ...
    msbuild mqusdRecorder.vcxproj /t:Build /p:Configuration=Release /p:Platform=Win32 /m /nologo
    IF %ERRORLEVEL% NEQ 0 (
        pause
        exit /B 1
    )
    ...
    mkdir "%DIST_DIR32%"
    mkdir "%CORE_DIR32%"
    copy _out\Win32_Release\%MQ_VERSION%\mqusdPlayer.dll "%DIST_DIR32%"
    copy _out\Win32_Release\%MQ_VERSION%\mqusdRecorder.dll "%DIST_DIR32%"
    copy _out\x64_Release\%MQ_VERSION%\mqusdCore.dll "%CORE_DIR32%"
    copy _out\x64_Release\%MQ_VERSION%\mqusdCore.exe "%CORE_DIR32%"
